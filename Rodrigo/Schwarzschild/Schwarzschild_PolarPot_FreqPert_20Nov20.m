...
(* ::Section:: *)
(*Quasi-normal Modes and quasi-normal functions Polar Pertu*)
...
n=(l-1)(l+2)/2;
V=(2*n^2*(n+1)*One+3*n^2*z+9*n*z^2/2+9*z^3/4)/(n+3*z/2)^2;
...
fn="Data/PolarPot/Spectra_N_"<>ToString[Nz]<>"_spin"<>ToString[spin]<>"_l"<>ToString[l]<>"_Freq_eps_"<>ToString[N[\[Epsilon]]]<>"_ksig_"<>ToString[kk]<>"_Prec_"<>ToString[Floor[Prec]]<>".dat";
Export[fn,N[SpectrumData,Prec],"Table"];

fn="Data/PolarPot/BranchSpectra_N_"<>ToString[Nz]<>"_spin"<>ToString[spin]<>"_l"<>ToString[l]<>"_Freq_eps_"<>ToString[N[\[Epsilon]]]<>"_ksig_"<>ToString[kk]<>"_Prec_"<>ToString[Floor[Prec]]<>".dat";
...
fn="Data/PolarPot/QNM_N_"<>ToString[Nz]<>"_spin"<>ToString[spin]<>"_l"<>ToString[l]<>"_Freq_eps_"<>ToString[N[\[Epsilon]]]<>"_ksig_"<>ToString[kk]<>"_Prec_"<>ToString[Floor[Prec]]<>".dat";
Export[fn,N[sData,30],"Table"];

fn=Table["Data/PolarPot/QNMSet"<>ToString[n]<>"_N_"<>ToString[Nz]<>"_spin"<>ToString[spin]<>"_l"<>ToString[l]<>"_Freq_eps_"<>ToString[N[\[Epsilon]]]<>"_ksig_"<>ToString[kk]<>"_Prec_"<>ToString[Floor[Prec]]<>".dat",{n,0,ntotal}];
For[n=0, n<=ntotal, n++,
