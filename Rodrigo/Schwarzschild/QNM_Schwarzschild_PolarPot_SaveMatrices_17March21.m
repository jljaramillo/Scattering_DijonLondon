...
n=(l-1)(l+2)/2;
VHighRes=(2*n^2*(n+1)*OneHighRes+3*n^2*\[Sigma]\[Sigma]+9*n*\[Sigma]\[Sigma]^2/2+9*\[Sigma]\[Sigma]^3/4)/(n+3*\[Sigma]\[Sigma]/2)^2;


One=ConstantArray[1,Nz+1];

V=(2*n^2*(n+1)*One+3*n^2*\[Sigma]+9*n*\[Sigma]^2/2+9*\[Sigma]^3/4)/(n+3*\[Sigma]/2)^2;
...
fn="OperatorMatrix/PolarParity/M_N_"<>ToString[Nz]<>"_Prec_"<>ToString[Floor[Prec]]<>".dat"
Export[fn,IntegerPart[10^(Prec+10)*M],"Table"];

fn="OperatorMatrix/PolarParity/MAdj0_N_"<>ToString[Nz]<>"_Prec_"<>ToString[Floor[Prec]]<>".dat"
Export[fn,IntegerPart[10^(Prec+10)*MAdj0],"Table"];

fn="OperatorMatrix/PolarParity/H0_N_"<>ToString[Nz]<>"_Prec_"<>ToString[Floor[Prec]]<>".dat"
